...
{    * Imementation of some methods with bindings dynamic for call the library }
{                                                                              }
{  IMPORTANT:                                                                  }
...
{=== Types For Methods with implementation dynamic  - modified in 3/Nov/2015  by m.h. ===}
  PxmlSecInit = function(): Longint; cdecl;
  PxmlSecCheckVersionExt = function(major: Longint; minor: Longint; subminor: Longint; mode: xmlSecCheckVersionMode): Longint; cdecl;
  PxmlSecCryptoDLLoadLibrary = function(const crypto: xmlCharPtr): Longint; cdecl;
  PxmlSecCryptoAppInit = function(const config: PAnsiChar): Longint; cdecl;
  PxmlSecCryptoInit = function(): Longint; cdecl;
  PxmlSecCryptoShutdown = function(): Longint; cdecl;
  PxmlSecCryptoAppShutdown = function(): Longint; cdecl;
  PxmlSecShutdown = function(): Longint; cdecl;
  PxmlSecFindNode = function(const parent: xmlNodePtr; const name: xmlCharPtr; const ns: xmlCharPtr): xmlNodePtr; cdecl;
  PxmlSecDSigCtxCreate = function(keysMngr: xmlSecKeysMngrPtr): xmlSecDSigCtxPtr; cdecl;
  PxmlSecCryptoAppKeyLoadMemory = function(const data: xmlSecBytePtr; dataSize: xmlSecSize; format: xmlSecKeyDataFormat; const pwd: PAnsiChar; pwdCallback: Pointer; pwdCallbackCtx: Pointer): xmlSecKeyPtr; cdecl;
  PxmlSecDSigCtxVerify = function(dsigCtx: xmlSecDSigCtxPtr; node: xmlNodePtr): Longint; cdecl;
  PxmlSecDSigCtxDestroy = procedure(dsigCtx: xmlSecDSigCtxPtr) cdecl;
  PxmlSecDSigCtxSign = function(dsigCtx: xmlSecDSigCtxPtr; tmpl: xmlNodePtr): Longint; cdecl;

{=== Methods Declarations - modified in 3/Nov/2015  by m.h. ===}
  function xmlSecInit(): Longint;
  function xmlSecCheckVersionExt(major: Longint; minor: Longint; subminor: Longint; mode: xmlSecCheckVersionMode): Longint;
  function xmlSecCryptoDLLoadLibrary(const crypto: xmlCharPtr): Longint;
  function xmlSecCryptoAppInit(const config: PAnsiChar): Longint;
  function xmlSecCryptoInit(): Longint;
  function xmlSecCryptoShutdown(): Longint;
  function xmlSecCryptoAppShutdown(): Longint;
  function xmlSecShutdown(): Longint;
  function xmlSecFindNode(const parent: xmlNodePtr; const name: xmlCharPtr; const ns: xmlCharPtr): xmlNodePtr;
  function xmlSecDSigCtxCreate(keysMngr: xmlSecKeysMngrPtr): xmlSecDSigCtxPtr;
  function xmlSecCryptoAppKeyLoadMemory(const data: xmlSecBytePtr; dataSize: xmlSecSize; format: xmlSecKeyDataFormat; const pwd: PAnsiChar; pwdCallback: Pointer; pwdCallbackCtx: Pointer): xmlSecKeyPtr;
  function xmlSecDSigCtxVerify(dsigCtx: xmlSecDSigCtxPtr; node: xmlNodePtr): Longint;
  procedure xmlSecDSigCtxDestroy(dsigCtx: xmlSecDSigCtxPtr);
  function xmlSecDSigCtxSign(dsigCtx: xmlSecDSigCtxPtr; tmpl: xmlNodePtr): Longint;
{=== End declarations of methods dynamics ===}
...
{=== Implementations Methods with loading dynamic - modified in 3/Nov/2015  by m.h. ===}
function xmlSecInit(): Longint;
var
  lHandle: TlibHandle;
...
begin
  ...
  if lHandle = NilHandle then
    Exit;
  try
    DoGetxmlSecInit := PxmlSecInit(GetProcAddress(lHandle, 'xmlSecInit'));
    if @DoGetxmlSecInit <> Nil then
      Result := DoGetxmlSecInit();
  finally
    FreeLibrary(lHandle);
  end;
end;
...
begin
  ...
  if lHandle = NilHandle then
    Exit;
  try
    DoGetxmlSecCheckVersionExt := PxmlSecCheckVersionExt(GetProcAddress(lHandle, 'xmlSecCheckVersionExt'));
    if @DoGetxmlSecCheckVersionExt <> Nil then
      Result := DoGetxmlSecCheckVersionExt(major, minor, subminor, mode);
  finally
    FreeLibrary(lHandle);
  end;
end;
...
begin
  ...
  if lHandle = NilHandle then
    Exit;
  try
    DoGetxmlSecCryptoDLLoadLibrary := PxmlSecCryptoDLLoadLibrary(GetProcAddress(lHandle, 'xmlSecCryptoDLLoadLibrary'));
    if @DoGetxmlSecCryptoDLLoadLibrary <> Nil then
      Result := DoGetxmlSecCryptoDLLoadLibrary(crypto);
  finally
    FreeLibrary(lHandle);
  end;
end;
...
begin
  ...
  if lHandle = NilHandle then
    Exit;
  try
    DoGetxmlSecCryptoAppInit := PxmlSecCryptoAppInit(GetProcAddress(lHandle, 'xmlSecCryptoAppInit'));
    if @DoGetxmlSecCryptoAppInit <> Nil then
      Result := DoGetxmlSecCryptoAppInit(config);
  finally
    FreeLibrary(lHandle);
  end;
end;
...
begin
  ...
  if lHandle = NilHandle then
    Exit;
  try
    DoGetxmlSecCryptoInit := PxmlSecCryptoInit(GetProcAddress(lHandle, 'xmlSecCryptoInit'));
    if @DoGetxmlSecCryptoInit <> Nil then
      Result := DoGetxmlSecCryptoInit();
  finally
    FreeLibrary(lHandle);
  end;
end;
...
begin
  ...
  if lHandle = NilHandle then
    Exit;
  try
    DoGetxmlSecCryptoShutdown := PxmlSecCryptoShutdown(GetProcAddress(lHandle, 'xmlSecCryptoShutdown'));
    if @DoGetxmlSecCryptoShutdown <> Nil then
      Result := DoGetxmlSecCryptoShutdown();
  finally
    FreeLibrary(lHandle);
  end;
end;
...
begin
  ...
  if lHandle = NilHandle then
    Exit;
  try
    DoGetxmlSecCryptoAppShutdown := PxmlSecCryptoAppShutdown(GetProcAddress(lHandle, 'xmlSecCryptoAppShutdown'));
    if @DoGetxmlSecCryptoAppShutdown <> Nil then
      Result := DoGetxmlSecCryptoAppShutdown();
  finally
    FreeLibrary(lHandle);
  end;
end;
...
begin
  ...
  if lHandle = NilHandle then
    Exit;
  try
    DoGetxmlSecShutdown := PxmlSecShutdown(GetProcAddress(lHandle, 'xmlSecShutdown'));
    if @DoGetxmlSecShutdown <> Nil then
      Result := DoGetxmlSecShutdown();
  finally
    FreeLibrary(lHandle);
  end;
end;
...
begin
  ...
  if lHandle = NilHandle then
    Exit;
  try
    DoGetxmlSecFindNode := PxmlSecFindNode(GetProcAddress(lHandle, 'xmlSecFindNode'));
    if @DoGetxmlSecFindNode <> Nil then
      Result := DoGetxmlSecFindNode(parent, name, ns);
  finally
    FreeLibrary(lHandle);
  end;
end;
...
begin
  ...
  if lHandle = NilHandle then
    Exit;
  try
    DoGetxmlSecDSigCtxCreate := PxmlSecDSigCtxCreate(GetProcAddress(lHandle, 'xmlSecDSigCtxCreate'));
    if @DoGetxmlSecDSigCtxCreate <> Nil then
      Result := DoGetxmlSecDSigCtxCreate(keysMngr);
  finally
    FreeLibrary(lHandle);
  end;
end;
...
begin
  ...
  if lHandle = NilHandle then
    Exit;
  try
    DoGetxmlSecCryptoAppKeyLoadMemory := PxmlSecCryptoAppKeyLoadMemory(GetProcAddress(lHandle, 'xmlSecCryptoAppKeyLoadMemory'));
    if @DoGetxmlSecCryptoAppKeyLoadMemory <> Nil then
      Result := DoGetxmlSecCryptoAppKeyLoadMemory(data, dataSize, format, pwd, pwdCallback, pwdCallbackCtx);
  finally
    FreeLibrary(lHandle);
  end;
end;
...
begin
  ...
  if lHandle = NilHandle then
    Exit;
  try
    DoGetxmlSecDSigCtxVerify := PxmlSecDSigCtxVerify(GetProcAddress(lHandle, 'xmlSecDSigCtxVerify'));
    if @DoGetxmlSecDSigCtxVerify <> Nil then
      Result := DoGetxmlSecDSigCtxVerify(dsigCtx, node);
  finally
    FreeLibrary(lHandle);
  end;
end;
...
begin
  ...
  if lHandle = NilHandle then
    Exit;
  try
    DoGetxmlSecDSigCtxDestroy := PxmlSecDSigCtxDestroy(GetProcAddress(lHandle, 'xmlSecDSigCtxDestroy'));
    if @DoGetxmlSecDSigCtxDestroy <> Nil then
      DoGetxmlSecDSigCtxDestroy(dsigCtx);
  finally
    FreeLibrary(lHandle);
  end;
end;
...
begin
  ...
  if lHandle = NilHandle then
    Exit;
  try
    DoGetxmlSecDSigCtxSign := PxmlSecDSigCtxSign(GetProcAddress(lHandle, 'xmlSecDSigCtxSign'));
    if @DoGetxmlSecDSigCtxSign <> Nil then
      Result := DoGetxmlSecDSigCtxSign(dsigCtx, tmpl);
  finally
    FreeLibrary(lHandle);
  end;
end;

{=== End implementations dynamics ===}
